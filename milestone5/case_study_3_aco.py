...
def case_study_3_aco(num_runs=1):
    """
    ...
    try:
        ...
        with open(report_path, 'w') as f:
            ...
            if best_solution:
                ...
                if hasattr(best_solution, 'combined_score') and best_solution.combined_score is not None:
                    f.write(f"  • Combined Score: {best_solution.combined_score:.4f} (lower = better)\n")
            ...
            if history:
                f.write("\nConvergence History:\n")
                f.write("Iteration | F1 (Coverage) | F2 (Imbalance) | Combined Score | Feasible Solutions\n")
                f.write("-" * 80 + "\n")
                for h in history:
                    combined = h.get('best_combined_score', 'N/A')
                    if isinstance(combined, (int, float)):
                        combined_str = f"{combined:.4f}"
                    else:
                        combined_str = str(combined)
                    f.write(f"{h['iteration']:9d} | {h.get('best_F1', 0):14d} | "
                           f"{h.get('best_F2', 0):15.2f} | {combined_str:14s} | "
                           f"{h.get('feasible_solutions', 0):17d}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run Case Study 3 with ACO')
    parser.add_argument('--runs', type=int, default=1, help='Number of runs for statistics (default: 1)')
    args = parser.parse_args()
    
    print("\n" + "="*80)
    print("ANT COLONY OPTIMIZATION - CASE STUDY 3")
    print("Testing ant3.py on Large Grid (10x10, 5 Robots)")
    if args.runs > 1:
        print(f"Running {args.runs} times for statistics")
    print("="*80)
    
    try:
        results = case_study_3_aco(num_runs=args.runs)
        if results:
            print("\n✅ Case Study 3 execution completed successfully!")
            print(f"Results saved in: milestone5/results/case_study_3_aco/")
        else:
